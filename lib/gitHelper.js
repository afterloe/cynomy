...
const [{resolve}, {spawn}, {existsSync, readdirSync, appendFile}] = [require("path"), require("child_process"), require("fs")];
const [createFile, {strToKeyValue}] = [require(resolve(__dirname, "createFile")), require(resolve(__dirname, "..", "tools", "currency"))];
...
const gitStatus = type => new Promise((solve, reject) => {
  ...
  awk.stderr.on('data', err => reject(new Error(err.toString())));
  awk.on("error", err => reject(err));
  awk.on("close", code => 0 === code ? solve(buf.toString()): reject(new Error(`git status -s | grep ${type} | awk '{print $2}' is failed`)));
});
...
const generatorHistoryMsg = function* (message) {
  const [userInfo, branchInfo] = yield [getGitUserInfo(), getBranchInfo()];
  return ``;
  // strToKeyValue
};
