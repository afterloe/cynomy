...
const [{join, resolve}, {writeFile, readdirSync}] = [require("path"), require("fs")];
const [createFile, {structureDir, setRoot}, {cp}] = [require(resolve(__dirname, "createFile")), require(resolve(__dirname, "structureDir")), require(resolve(__dirname, "..", "tools", "fileSystem"))];
...
const extendPkg = root => new Promise((resolve, reject) => {
    const pkg = require(join(root, "package"));
    Object.assign(pkg, {
      "devDependencies": {
        "istanbul": "^0.4.5",
        "jshint": "^2.9.4",
...
const createCynomy = root => new Promise((resolve, reject) => {
    const {author = process.env.LOGNAME, license = "", version = "1.0.0", mail = "", homePage = ""} = require(join(root, "package"));
    const cynomyCfg = {author, licensed: license, mail, homePage, version, };
...
const autoComplateFile = root => {
  ...
  // 构建 test文件夹
    setRoot(root);
    const taskList = Promise.all([createCynomy(root), createMakefile(root), cp(resolve(__dirname, "..", ".jshintrc"), root),
      cp(resolve(__dirname, "..", ".tern-project"), root), extendPkg(root), structureDir("test"), structureDir("doc"), structureDir("config")]);
    taskList.then(() => {
