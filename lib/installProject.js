...
const autoComplateFile = root => {
  ...
  // 拷贝 .tern-project
  // 扩展 package.json
    const taskList = Promise.all([createCynomy(root), cp(".jshintrc", root), cp(".tern-project", root), extendPkg(root)]);
    taskList.then(() => {
        console.log("file is copy success!");
    }).catch(err => {
        console.log(err);
    });
};
...
module.exports = ({frame}) => {
    ...
    console.log("begin to create file good luck...");
