...
// 申明文件内容职责链
const [jsNode, htmlNode, cssNode, bashNode, pythonNode ,binaryNode] = [new Chain(createJSFile),
  new Chain(createHTMLFile), new Chain(createCssFile), new Chain(createBashFile),
  new Chain(createPyhtonFile), new Chain(createBinaryFile)];

// 设置文件内容职责链
jsNode.setNext(htmlNode).setNext(cssNode).setNext(bashNode).setNext(pythonNode).setNext(binaryNode);
